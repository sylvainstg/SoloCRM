...
exports.onGmailPush = exports.setupGmailWatch = exports.storeGmailTokens = void 0;
const https_1 = require("firebase-functions/v2/https");
const pubsub_1 = require("firebase-functions/v2/pubsub");
const admin = require("firebase-admin");
const functions = require("firebase-functions"); // Keep for config() if needed, or switch to params
 ...
 */
exports.storeGmailTokens = (0, https_1.onCall)(async (request) => {
    if (!request.auth)
 ...
 */
exports.setupGmailWatch = (0, https_1.onCall)(async (request) => {
    if (!request.auth)
        throw new https_1.HttpsError('unauthenticated', 'User must be logged in');
    ...
    if (!token)
        throw new https_1.HttpsError('failed-precondition', 'No access token available');
 ...
 */
exports.onGmailPush = (0, pubsub_1.onMessagePublished)("gmail-updates", async (event) => {
...
async function processHistory(uid, newHistoryId) {
    // 1. Get Tokens
    ...
    const oauth2Client = new googleapis_1.google.auth.OAuth2(functions.config().google.client_id, functions.config().google.client_secret);
...
async function processSingleMessage(uid, gmail, messageId) {
    // Fetch full details
    ...
    if (labelIds.includes('SENT'))
        return; // Ignore own emails for now? User said "Incoming"
    // Search Contact
