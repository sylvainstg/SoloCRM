...
const EmailSyncView: React.FC<Props> = ({
  ...
  const handleLog = async (thread: EmailThread, e: React.MouseEvent) => {
    ...
    if (contact) {
      ...
      // Critical: Update local state immediately before waiting for async delete
      setLoggedIds(prev => new Set(prev).add(thread.id));
      setTempLogged(prev => new Map(prev).set(thread.id, thread.snippet));

      // Remove from Triage Queue (Firestore)
      try {
        await deleteTriageItem(user.uid, thread.id);
      } catch (e) {
        console.error("Failed to delete triage item:", e);
        // Optional: Revert local state if delete fails, but for now we prioritized UI responsiveness
      }
    } else {
